...
class MenuSystem
{
	...
	$xdelay = 350, $xduration = 350, $xtransition = 'Fx.Transitions.linear')
	{
		...
		if($fancy)
		{
			...
			$this->fancy = '<script language="javascript" type="text/javascript">window.addEvent("load", function(){new ZTFancy($E("ul", "zt-mainmenu-inner"), {transition: '.$transition.', duration: '.$duration.', onClick: function(ev, item){ev.stop();}});});</script>';
		}
	}
	...
	function show($start=0, $end = 14)
	{
		...
		if($this->_start == 0)
		{
			($this->_name != 'mega') ? $this->showMenu(0, 0) : $this->showMegaMenu(0, 0);
		}
		else
		{
			$parID = $this->fatherId($this->_start); 
			if($parID)($this->_name != 'mega') ? $this->showMenu($parID, $this->_start) : $this->showMegaMenu($parID, $this->_start);
		}
	...
	*/
	...
	function genMegaTypeNormal($row, $level = 0)
	{
		...
		$group	= $this->getMegaMenuParam($params, "mega_group", 0);
		$width	= $this->getMegaMenuParam($params, "mega_width", '200px');
		$colw	= $this->getMegaMenuParam($params, "mega_colw", '200px');
		$colxw	= $this->getMegaMenuParam($params, "mega_colxw", '');
